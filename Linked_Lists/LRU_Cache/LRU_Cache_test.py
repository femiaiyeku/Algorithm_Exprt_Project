...
class  TestProgram(unittest.TestCase):
    def test_case_1(self):
        lru_cache = ProgrammingError.LRUCache(3)
        lru_cache.insertKeyValuePair("b", 2)
        lru_cache.insertKeyValuePair("a", 1)
        lru_cache.insertKeyValuePair("c", 3)
        self.assertEqual(lru_cache.getMostRecentKey(), "c")
        self.assertEqual(lru_cache.getValueFromKey("a"), 1)
        self.assertEqual(lru_cache.getMostRecentKey(), "a")
        lru_cache.insertKeyValuePair("d", 4)
        self.assertEqual(lru_cache.getValueFromKey("b"), None)
        lru_cache.insertKeyValuePair("a", 5)
        self.assertEqual(lru_cache.getValueFromKey("a"), 5)
