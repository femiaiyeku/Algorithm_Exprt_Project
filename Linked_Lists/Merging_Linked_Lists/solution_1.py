...
def intersection(headOne, headTwo):
    # Write your code here.
    lenOne = getLen(headOne)
    lenTwo = getLen(headTwo)
    if lenOne > lenTwo:
        return getIntersection(headOne, headTwo, lenOne - lenTwo)
    else:
        return getIntersection(headTwo, headOne, lenTwo - lenOne)
    
def getIntersection(headOne, headTwo, diff):
    while diff > 0:
