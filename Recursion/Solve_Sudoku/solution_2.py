...
def solveSudoku(board):
    ...
    solvePartialSudoku(0, 0, board, rows, cols, squares, digits)
    return board

def solvePartialSudoku(row, col, board, rows, cols, squares, digits):
    if row == 9:
        return True
    ...
    if board[row][col] == 0:
        return tryDigitsAtPosition(row, col, board, rows, cols, squares, digits)
    return solvePartialSudoku(nextRow, nextCol, board, rows, cols, squares, digits)

def tryDigitsAtPosition(row, col, board, rows, cols, squares, digits):
    for digit in digits:
        if isValidAtPosition(digit, row, col, board, rows, cols, squares):
            board[row][col] = digit
            rows[row].add(digit)
            cols[col].add(digit)
            squares[row // 3][col // 3].add(digit)
            if solvePartialSudoku(row, col, board, rows, cols, squares, digits):
                return True
            board[row][col] = 0
            rows[row].remove(digit)
            cols[col].remove(digit)
            squares[row // 3][col // 3].remove(digit)
    return False

def isValidAtPosition(value, row, col, board, rows, cols, squares):
    rowValid = value not in rows[row]
    colValid = value not in cols[col]
    squareValid = value not in squares[row // 3][col // 3]
    return rowValid and colValid and squareValid


#O(1) time | O(1) space - where n is the number of elements in the matrix
