...
def ambiguousMeasurements(cups, low, high):
    failedCombinations = set()
    return canMeasureInRange(cups, low, high, 0, 0, failedCombinations)

def canMeasureInRange(cups, low, high, currentLow, currentHigh, failedCombinations):
    key = (currentLow, currentHigh)
    if key in failedCombinations:
        return False
    ...
    for cup in cups:
        cupLow, cupHigh = cup
        newLow = currentLow + cupLow
        newHigh = currentHigh + cupHigh
        if low <= newLow and newHigh <= high:
            canMeasure = True
            break

        if canMeasureInRange(cups, low, high, newLow, newHigh, failedCombinations):
            canMeasure = True
            break
