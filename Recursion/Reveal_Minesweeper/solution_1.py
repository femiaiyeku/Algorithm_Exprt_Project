...
def revealMinesweeper(board, click):
    row, col = click
    if board[row][col] == "M":
        board[row][col] = "X"
    else:
        revealEmptySquares(board, row, col)
    return board

def revealEmptySquares(board, row, col):
    if board[row][col] != "E":
        return
    numAdjacentMines = getNumAdjacentMines(board, row, col)
    if numAdjacentMines > 0:
        board[row][col] = str(numAdjacentMines)
        return
    board[row][col] = "B"
    for rowIdx in range(row - 1, row + 2):
        for colIdx in range(col - 1, col + 2):
            ...
            if rowIdx < 0 or rowIdx >= len(board) or colIdx < 0 or colIdx >= len(board[0]):
                continue
            revealEmptySquares(board, rowIdx, colIdx)

def getNumAdjacentMines(board, row, col):
    numAdjacentMines = 0
    for rowIdx in range(row - 1, row + 2):
        for colIdx in range(col - 1, col + 2):
            if rowIdx == row and colIdx == col:
