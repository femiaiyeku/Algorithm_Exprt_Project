...
def rightSiblingTree(root):
    transform_to_right_sibling_tree(root, None, None)
    return root

def transform_to_right_sibling_tree(node, parent, is_left_child):
    if node is None:
        return
    left, right = node.left, node.right
    transform_to_right_sibling_tree(left, node, True)
    if parent is None:
        node.right = None
    elif is_left_child:
    ...
    transform_to_right_sibling_tree(right, node, False)
