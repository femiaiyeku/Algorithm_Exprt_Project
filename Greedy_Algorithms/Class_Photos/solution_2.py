"""
It's photo day at the local school, and you're the photographer assigned to take class photos. The class that you'll be
...
students in two rows before taking the photo. Each row should contain the same number of the students and should
...
the heights of all the students with blue shirts. These arrays will always have the same length, and each height will be a
...
def classPhotos(redShirtHeights, blueShirtHeights):
    ...
    bool_list = list(map(lambda x, y: x > y, redShirtHeights, blueShirtHeights))
    if all(bool_list) or not any(bool_list):
        return False
    else:
        return True
