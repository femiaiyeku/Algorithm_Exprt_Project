...
def largestIsland(matrix):
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[0])):
            if matrix[i][j] == 0:
                size = getConnectedIslandsSize(matrix, i, j, islandSizes)
                maxSize = max(maxSize, size)

    return maxSize


def dfs(matrix, i, j, islandId):
    if i < 0 or i >= len(matrix) or j < 0 or j >= len(matrix[0]) or matrix[i][j] != 1:
        return 0

    matrix[i][j] = islandId
    size = 1
    size += dfs(matrix, i+1, j, islandId)
    size += dfs(matrix, i-1, j, islandId)
    size += dfs(matrix, i, j+1, islandId)
    size += dfs(matrix, i, j-1, islandId)

    return size


def getConnectedIslandsSize(matrix, i, j, islandSizes):
    connectedIslands = set()
    if i > 0:
        connectedIslands.add(matrix[i-1][j])
    if i < len(matrix) - 1:
        connectedIslands.add(matrix[i+1][j])
    if j > 0:
        connectedIslands.add(matrix[i][j-1])
    if j < len(matrix[0]) - 1:
        connectedIslands.add(matrix[i][j+1])

    size = 1
    for islandId in connectedIslands:
        size += islandSizes[islandId]
