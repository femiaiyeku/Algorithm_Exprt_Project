...
def removeIslands(matrix):
    ...
    for x in [0, len(matrix) - 1]:
        for y in range(len(matrix[x])):
            if matrix[x][y] == 1:
                changeOnesConnectedToBorderToTwos(matrix, x, y)
...
def changeOnesConnectedToBorderToTwos(matrix, startX, startY):
    stack = [(startX, startY)]
    ...
    while len(stack) > 0:
        ...
        neighbors = getNeighbors(matrix, currentX, currentY)
